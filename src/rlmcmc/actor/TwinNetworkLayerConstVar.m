classdef TwinNetworkLayerConstVar < nnet.layer.Layer
    ...
    properties (Learnable)
        ...
        %}
        mean_drift_weights_input_hidden;
        mean_drift_bias_hidden;
        mean_drift_weights_hidden_output;
    ...
    methods
        ...
        function layer = TwinNetworkLayerConstVar(args)
            %{
            ...
            layer.mean_drift_weights_input_hidden = layer.xavier_uniform_init(args.input_nodes, args.hidden_nodes);
            layer.mean_drift_bias_hidden = zeros(args.hidden_nodes, 1);
            layer.mean_drift_weights_hidden_output = layer.xavier_uniform_init(args.hidden_nodes, args.output_nodes);
        ...
        function res = mean_drift_nn(layer, X)
            ...
            Args:
                X: input array.
            %}
            X = layer.linear(X, layer.mean_drift_weights_input_hidden, layer.mean_drift_bias_hidden);
            X = layer.relu(X);
            X = layer.linear(X, layer.mean_drift_weights_hidden_output, layer.mean_drift_bias_output);
            res = X;
        end
