...
from ..utils import MCMCAnimation
...
class RunningFactoryInterface(ABC):
    def __init__(
        ...
        assert mode in ["ddpg", "td3", "ddpg_random"], "mode must be ddpg, td3, or ddpg_random."
    ...
    def plot_action(
            ...
            num: int = 1000,
            xlim: tuple[Union[float, int], Union[float, int]] = (-5., 5.),
            ylim: tuple[Union[float, int], Union[float, int]] = (-5., 5.),
        ...
        #         pdf_res[i, j] = np.exp(log_target_pdf(np.array([x_target[i], y_target[j]])))
        ...
        # ax.set_aspect('equal')
        # plt.savefig('pic/select_equal.png', dpi=100)
        # plt.tight_layout()
        pass
...
class RunningFactory(RunningFactoryInterface):
    ...
    def run(
        ...
        wandb.init(
            project=self.project_name, config=dataclasses.asdict(self.learning_factory.args)
        )
        ...
        # Prediction
        prediction = self.learning.predict(predicted_env=self.learning_factory.predicted_envs)
        prediction.plot()
