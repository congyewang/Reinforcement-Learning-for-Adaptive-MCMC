...
%% Add Log Target PDF
log_target_pdf = @(x) multi_modal_gaussian_target(x);

%% get (approx) mean (mu) and covariance (Sigma) from adaptive mcmc
pretrain_nits = 1000;
pretrain_sample = unifrnd(-15, 10, [pretrain_nits, 1]); % data for pre-training
...
policy_plot(policy1, "Step 1 Policy", -10, 10);
axis square;
...
policy_plot(policy3, "Step 150 Policy", -10, 10);
axis square;
...
policy_plot(policy140, "Step 70000 Policy", -10, 10);
axis square;
