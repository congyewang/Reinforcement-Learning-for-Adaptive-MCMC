...
class RLMHEnv(gym.Env):
    def __init__(self, log_target_pdf, sample_dim=2, total_timesteps=10_000):
        ...
        self.observation_space = spaces.Box(low=-INF, high=INF, shape=(1, 3*sample_dim), dtype=np.float64)
    ...
    def step(self, action):
        ...
        # mcmc_noise = self.state[:, self.sample_dim:2*self.sample_dim]
        # policy_noise = self.state[:, 2*self.sample_dim:]
        ...
        policy_noise = self.np_random.normal(size=(1, self.sample_dim))
        state = np.hstack((accepted_sample, mcmc_noise, policy_noise))
    ...
    def reset(self, seed=None, options=None):
        ...
        self.state = np.hstack(
            (
                np.zeros((1, self.sample_dim)), # Initial Sample
                self.np_random.normal(size=(1, self.sample_dim)), # MCMC Noise
                self.np_random.normal(size=(1, self.sample_dim)) # Policy Noise
            )
        )
